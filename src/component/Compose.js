...
export default function Compose() {
  ...
  const handleMailSent = (newMail) => {
    ...
    console.log(newMail)
    // const dataArrayA = Object.keys(newMail).map((key) => ({ id: key, ...newMail[key] }));
  ...
  return (
    <>
      <div>
        <div className="box float-left bg-gray-400 w-1/5 min-h-screen p-4">
          <button
            className="w-4/5 text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-lg text-center p-2.5 mb-4"
            onClick={handleComposeOpen}
          >
            Compose
          </button>
                ...
                <ul className="space-y-4">
                  {mails.map((mail) => (
                    ...
                    >
                      ...
                      <p className="font-semibold">Subject: {mail.subject}</p>
                      <p className="text-gray-700">Body: {mail.body}</p>
                      {/* <p className="font-bold">id:{mail.id}</p> */}
                      {!mail.read && (
                        <button
                          className="bg-blue-500 px-2 border border-1 mt-2 rounded-md mx-2"
                          onClick={() => markAsRead(mail.id)}
      ...
      {isComposeOpen && (
        <Mail onClose={handleComposeClose} onMailSent={handleMailSent} />
      )}
    </>
  );
}
